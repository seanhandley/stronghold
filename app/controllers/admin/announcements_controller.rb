class Admin::AnnouncementsController < AdminBaseController
  ...
  def create
    ...
    if @announcement.save
      redirect_to admin_announcements_path, notice: 'Announcement created successfully'
    else
  ...
  def destroy
    ...
    if announcement.save
      redirect_to admin_announcements_path, notice: "Successfully deactivated."
    else
      flash[:error] = @announcement.errors.full_messages.join('<br>')
      respond_to do |format|
        format.html {
