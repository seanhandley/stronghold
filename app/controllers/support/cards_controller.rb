  ...
  def create
    ...
    if @customer_signup.ready?
      ...
      Hipchat.notify('Self Service', 'Accounts',
                     "User completed signup: #{@customer_signup.email} becoming organization #{current_user.organization.name}. Voucher: #{create_params[:discount_code].present? ? create_params[:discount_code] : 'N/A'}",
                     color: 'green')
