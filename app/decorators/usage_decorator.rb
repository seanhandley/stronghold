class UsageDecorator < ApplicationDecorator
  ...
  def usage_data(args=nil)
    ...
    if key.include?(full_month_cache_stamp(from_date))
      ...
      if usage
        return present_with_tenants(usage.usage_data)
      else
      ...
      if(tenant_id == tenant.id)
        if results[:ip_quota_results].none?
          ...
          cost = results[:ip_quota_results].collect do |quota|
            period = ((((Time.parse(quota[:recorded_at].to_s) - start) / 60.0) / 60.0) / 24.0).round
            start = Time.parse(quota[:recorded_at].to_s)
          ...
          period = ((((to_date - Time.parse(results[:ip_quota_results].last[:recorded_at].to_s)) / 60.0) / 60.0) / 24.0).round
          total_rate = (period * daily_rate)
          cost += (q * total_rate)
          cost
        end
