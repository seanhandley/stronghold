      ...
      if(tenant_id == tenant.id)
        if results[:ip_quota_results].none?
          ...
          cost = results[:ip_quota_results].collect do |quota|
            period = ((((Time.parse(quota[:recorded_at]) - start) / 60.0) / 60.0) / 24.0).round
            start = Time.parse(quota[:recorded_at])
          ...
          period = ((((to_date - Time.parse(results[:ip_quota_results].last[:recorded_at])) / 60.0) / 60.0) / 24.0).round
          total_rate = (period * daily_rate)
          cost += (q * total_rate)
          cost
        end
