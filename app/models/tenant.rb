class Tenant < ActiveRecord::Base
  ...
  def compute_quota
    Rails.cache.fetch("compute_quotas_for_#{uuid}", expires_in: 1.hour.from_now) do
  ...
  def volume_quota
    Rails.cache.fetch("volume_quotas_for_#{uuid}", expires_in: 1.hour.from_now) do
  ...
  def network_quota
    Rails.cache.fetch("network_quotas_for_#{uuid}", expires_in: 1.hour.from_now) do
