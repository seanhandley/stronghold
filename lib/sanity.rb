module Sanity
  def self.check
    ...
    if previous_results
      ...
      cache.write(key, results, expires_in: 3.days.from_now)
  ...
  def self.live_instances
    Rails.cache.fetch('sanity_live_instances', expires_in: 10.minutes.from_now) do
  ...
  def self.live_images
    Rails.cache.fetch('sanity_live_images', expires_in: 10.minutes.from_now) do
      OpenStackConnection.compute.list_images.body['images'].collect{|image| image['id']}
    end
  end

  def self.live_routers
    Rails.cache.fetch('sanity_live_routers', expires_in: 10.minutes.from_now) do
