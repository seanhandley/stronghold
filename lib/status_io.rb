...
module StatusIO
  ...
  def self.active_incidents
    Rails.cache.fetch("status_io_active_incidents", expires_in: 5.minutes.from_now) do
      list_items('incident', 'active_incidents')
    end
  end

  def self.active_maintenances
    Rails.cache.fetch("status_io_active_maintenances", expires_in: 5.minutes.from_now) do
      list_items('maintenance', 'active_maintenances')
    end
  end

  def self.upcoming_maintenances
    Rails.cache.fetch("status_io_upcoming_maintenances", expires_in: 5.minutes.from_now) do
