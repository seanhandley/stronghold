...
class Support::ChangeOrganizationsControllerTest < CleanControllerTest
  setup do
    ...
    @user = User.make!(organizations: [@active_org, @active_org2, @frozen_org, @fresh_org])
    log_in(@user)
  end
  ...
  test "user can't change organizations to a closed account" do
    old = session[:organization_id]
    put :change, params: { organization_id: @closed_org.id}
    assert_redirected_to support_root_path
  ...
  test "user can't change organizations to an organization they do not belong to" do
    old = session[:organization_id]
    assert_not @user.organizations.include?(@forbidden_org)
