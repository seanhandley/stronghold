...
class MailerTest < ActionMailer::TestCase
  def test_signup
    ...
    assert email.body.raw_source.include?(@invite.token), 'mail body contains invite token'
  end
  ...
  def test_reset
    ...
    assert email.body.raw_source.include?(@reset.token), 'mail body contains reset token'
  end
  ...
  def test_usage_sanity_failures
    ...
    assert email.text_part.to_s.include?(text), 'message body contains details'
  end
  ...
  def test_fraud_check_alert
    ...
    assert @email.body.raw_source.include?('MaxMind Risk Score:</strong> 89.29 (out of 100)')
  end
